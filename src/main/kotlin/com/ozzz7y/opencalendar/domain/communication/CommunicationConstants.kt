package com.ozzz7y.opencalendar.domain.communication

/**
 * The constants used for communication.
 */
object CommunicationConstants {

    /**
     * The API prefix.
     */
    const val API: String = "api"

    /**
     * The API version.
     */
    const val API_VERSION: String = "v1"

    /**
     * The development URL for the frontend.
     */
    const val FRONTEND_DEVELOPMENT_URL: String = "http://localhost:5173"

    /**
     * The production URL for the frontend.
     */
    const val FRONTEND_PRODUCTION_URL: String = "https://open-calendar-frontend.onrender.com"

}
